  ...
  return (
    <div className="fixed bottom-0 inset-x-0 z-[60] p-3 sm:p-4">
      <div className="max-w-3xl mx-auto rounded-2xl border border-slate-200 bg-white shadow-lg p-4 flex flex-col sm:flex-row items-start sm:items-center gap-3">
        <Cookie className="w-5 h-5 text-teal-600 shrink-0 mt-0.5 sm:mt-0" />
        <p className="text-sm text-slate-600 flex-1">
          Мы используем только необходимые cookies для работы сайта и входа в аккаунт. Подробнее — в{' '}
          <Link to="/privacy" className="text-teal-600 hover:underline">Политике конфиденциальности</Link>.
        </p>
        <button onClick={accept}
          className="shrink-0 h-9 px-5 rounded-xl bg-teal-500 text-white text-sm font-medium hover:bg-teal-600 transition-colors">
          Принять
        </button>
      </div>
    </div>
