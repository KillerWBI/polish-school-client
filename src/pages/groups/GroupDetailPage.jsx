  ...
  return (
    <div className="p-5 sm:p-8">
      <button onClick={() => navigate('/groups')}
        className="flex items-center gap-1.5 text-sm text-slate-400 hover:text-white mb-4 cursor-pointer transition-colors">
        ...
        {TABS.map((t, i) => (
          <button key={t} onClick={() => setTab(i)}
            className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all cursor-pointer ${
              tab === i ? 'bg-brand-600/30 text-brand-300' : 'text-slate-400 hover:text-white'
            }`}>
            {t}
          </button>
        ))}
...
function AddStudentModal({ open, onClose, groupId, existing, onAdded }) {
  ...
  return (
    <Modal open={open} onClose={onClose} maxWidth="max-w-sm">
      <div className="p-6">
        <h3 className="text-lg font-semibold text-white mb-4">Добавить студента</h3>
        ...
        {lessons.map(l => (
          <button key={l.id} onClick={() => onSelect(l)}
            className={`w-full text-left flex items-center gap-4 px-4 py-3 rounded-xl border transition-all cursor-pointer group ${
              muted
                ? 'bg-white/[0.02] border-white/[0.05] opacity-60 hover:opacity-100 hover:bg-white/[0.04]'
                : 'bg-white/[0.04] border-white/[0.07] hover:bg-white/[0.07] hover:border-brand-500/30'
            }`}>
            {/* Дата */}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                {l.lessonLink && (
                  <span className="text-xs text-brand-400">● ссылка</span>
                )}
            ...
            <div className="flex items-center gap-2 shrink-0">
              {l.Homeworks?.length > 0 && (
                <span className="text-xs px-2 py-0.5 rounded-full bg-brand-700/30 text-brand-300">
    ...
    <Modal open onClose={onClose} maxWidth="max-w-md">
      <div className="p-6">
        ...
        <div className="flex items-start justify-between gap-3 mb-5">
          ...
          <button onClick={onClose} className="text-slate-500 hover:text-white cursor-pointer shrink-0 p-1">
        ...
        {/* Описание */}
        {lesson.description && (
          <p className="text-sm text-slate-300 mb-4 leading-relaxed">{lesson.description}</p>
        )}

        {/* Ссылка на урок */}
        {linkUrl && (
          <a href={linkUrl} target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-brand-600/20 border border-brand-600/30 text-brand-300 text-sm hover:bg-brand-600/30 transition-colors mb-4">
            <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
        ...
        {/* Кнопки учителя */}
        {isTeacher && (
          <div className="flex gap-2 pt-2 border-t border-white/[0.07] mt-4">
...
function EditLessonForm({ lesson, onSaved, onCancel }) {
  ...
  return (
    <div className="p-6 sm:p-7">
      <h3 className="text-lg font-semibold text-white mb-5">Редактировать урок</h3>
      <form onSubmit={handleSubmit} className="space-y-3">
...
function CreateLessonModal({ open, onClose, groupId, onCreated }) {
  ...
  return (
    <Modal open={open} onClose={handleClose} maxWidth="max-w-lg">
      <div className="p-6 sm:p-7">
        <h3 className="text-lg font-semibold text-white mb-5">Новый урок</h3>
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-slate-400 block mb-1">Дата *</label>
...
function GenerateLessonsModal({ open, onClose, group, onGenerated }) {
  ...
  return (
    <Modal open={open} onClose={handleClose} maxWidth="max-w-sm">
      <div className="p-6">
        <h3 className="text-lg font-semibold text-white mb-2">Сгенерировать по расписанию</h3>
        {schedule && (
          <p className="text-xs text-slate-400 mb-5">Расписание: {schedule}</p>
        )}
        {!schedule && (
          <p className="text-xs text-amber-400 mb-5">⚠️ У группы нет расписания. Сначала добавьте его в «Настройках».</p>
        )}
        {result !== null ? (
          <div className="text-center py-4">
            <div className="text-4xl mb-3">✅</div>
            <p className="text-white font-medium">Создано {result} уроков</p>
          ...
          <div key={i} className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.08] space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => remove(i)}
                className="text-slate-600 hover:text-red-400 cursor-pointer text-xs">✕</button>
            </div>
            <input placeholder="Заголовок (необязательно)" value={m.title || ''}
              onChange={e => update(i, 'title', e.target.value)}
              className="w-full h-8 px-3 rounded-lg bg-white/[0.07] border border-white/[0.10] text-white text-xs outline-none focus:border-brand-400 placeholder:text-slate-600" />
            {m.type === 'link' ? (
              ...
              <textarea placeholder="Текст заметки..." value={m.content || ''}
                ...
                className="w-full px-3 py-2 rounded-lg bg-white/[0.07] border border-white/[0.10] text-white text-xs outline-none focus:border-brand-400 placeholder:text-slate-600 resize-none" />
            )}
          </div>
        ))}
      </div>
...
function MaterialsList({ materials }) {
  return (
    <div className="space-y-2">
      {materials.map((m, i) => (
        <div key={i} className="flex items-start gap-2.5 px-3 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.07]">
          <span className="text-base shrink-0 mt-0.5">
            {m.type === 'link' ? '🔗' : m.type === 'file' ? '📎' : '📝'}
          </span>
          <div className="flex-1 min-w-0">
            {m.title && <p className="text-xs font-medium text-white mb-0.5">{m.title}</p>}
            {m.type === 'link' && m.url && (
