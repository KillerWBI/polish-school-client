...
| AdminPage (вкладки Обзор/Пользователи; роли/планы/деактивация) | `/admin` | ✅ | ✅ Lazy |

---

## Что осталось (актуально 2026-07-10)

### 🔴 Срочно (перед публичным запуском)
- [ ] **Railway прод:** прогнать `npm run db:migrate` (миграции `20260709000001` + `20260709000002`)
- [ ] **Railway прод:** задать `ADMIN_EMAIL=<email>` в env Variables
- [ ] **Ротация секретов** — Cloudinary/Resend/пароль БД утекли в git-историю

### 🟡 Важно — новые фичи для ученика (разработка)
- [ ] История оплат ученика (таб «История» в `/payments` — уже есть `GET /payments/history` на бэке)
- [ ] Заметки к урокам (`StudentNote` — личный дневник)
- [ ] Личный словарь (`VocabItem` + SR-алгоритм)
- [ ] Страница прогресса ученика `/my-progress`
- [ ] Трекер внешних уроков (`StudentLessonLog` — уроки вне платформы)
- [ ] In-app уведомления (`Notification` model + `NotifBell`)
- [ ] Материалы урока `/materials` (обе роли)

### 🟡 Расширение админки
- [ ] Вкладка «Подписки/Доходы» в AdminPage
- [ ] Вкладка «Поддержка» (SupportTicket + ответ по email)

### ⚪ Низкий приоритет
- [ ] **TanStack Query** вместо `useFetch` — кэш между маршрутами, авто-refetch. Крупная задача.
- [ ] **Recharts lazy в Dashboard** — AnalyticsChart статически импортируется в DashboardPage → Recharts в main bundle. Решение: `const AnalyticsChart = lazy(...)` внутри DashboardPage.
- [ ] **og.png** (1200×630) — нужен для OG-карточки перед анонсом
- [ ] Экспорт PDF/Excel
- [ ] Абонемент уроков ученика (пакет N уроков)

### ⏸️ Отложено
- [ ] Онлайн-оплата занятий (Stripe/BLIK/P24) — шлюз не выбран, нужен домен
- [ ] Оплата подписки-тарифа (биллинг) — ждёт тот же шлюз
- [ ] Free plan лимиты — показываются, не блокируются (сознательно)
- [ ] School mode (несколько учителей под одним брендом)
- [ ] Домен + доменная почта для Resend (нужны для production email)
